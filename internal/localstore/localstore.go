...
func ensureSchema(db *sql.DB) error {
	...
	// Add last_indexed_commit column on existing databases (graceful upgrade).
	_, _ = db.Exec(`ALTER TABLE projects ADD COLUMN last_indexed_commit TEXT NOT NULL DEFAULT ''`)
	return nil
}
...
// UpdateProjectCommit stores the last indexed commit SHA.
...
// through the dependency graph. SQLite supports WITH RECURSIVE since 3.8.3.
