...
func TestPrivacyRoutingTextOnlyWhenNoLocalProvider(t *testing.T) {
	...
	fs := &fakeStore{}
	idx := NewIndexer(fs, &fakeEmbedder{localAvailable: false}, 3, 4, 8, false, false, "")
...
func TestPrivacyRoutingEmbedsLocallyWhenAvailable(t *testing.T) {
	...
	fs := &fakeStore{}
	idx := NewIndexer(fs, &fakeEmbedder{localAvailable: true}, 3, 4, 8, false, false, "")
...
func TestIndexProjectStopsOnCancel(t *testing.T) {
	...
	idx := NewIndexer(&fakeStore{}, emb, 3, 4, 8, false, false, "")
...
func TestConcurrentIndexingIsComplete(t *testing.T) {
	...
	fs := &fakeStore{}
	idx := NewIndexer(fs, &fakeEmbedder{}, 3, 8, 8, false, false, "")
...
func TestWorkerPoolParallelizes(t *testing.T) {
	run := func(workers int) time.Duration {
		...
		idx := NewIndexer(&fakeStore{}, emb, 3, workers, 8, false, false, "")
		start := time.Now()
		if _, err := idx.IndexProject(context.Background(), 1, dir, "bge-m3", 0); err != nil {
			t.Fatalf("IndexProject: %v", err)
		}
...
func TestKeywordOnlyStoresTextWithoutEmbedding(t *testing.T) {
	...
	idx := NewIndexer(fs, emb, 1, 4, 8, false, false, "").SetKeywordOnly(true)
