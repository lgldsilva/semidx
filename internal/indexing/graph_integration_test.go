...
`)
	...
	// service-a/main.go imports both its own nested module (re-anchored to the
	// project root) and the root module (already project-relative): a monorepo
	// keeps both edges.
	wantDeps := map[string][]string{
		"root.go":           {"pkg/shared/"},
		"service-a/main.go": {"service-a/internal/lib/", "pkg/shared/"},
	}
