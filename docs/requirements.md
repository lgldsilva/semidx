...
| REQ-SRCH-09 | Human and grep-friendly formatters with `file:line` and understandable scores | **done** | Human formatter shows explicit rank score (not a probability); sgrep `file:line:content` contract preserved |
...
| REQ-NFR-03 | Toolchain | Go **1.27.0** pin (`GOTOOLCHAIN`; 1.27.0 required by `charm.land/fantasy` ≥0.42); bump builder image with stdlib |
...
Ordered for product leverage. Items marked *(done)* are delivered — kept here for traceability.

### Phase A — Finish the daily product loop *(mostly done)*

1. ~~REQ-UI-07 bulk folder/archive ingest~~ *(done)*
2. ~~REQ-UI-08 file detail (chunks + fan-in/fan-out)~~ *(done)*
3. ~~REQ-SRV-10 richer live jobs~~ *(done)*
4. REQ-PARITY-01/02 matrix accuracy; CLI-only policy for alerts/insights *(partial)*
5. REQ-UI-06/UX polish (actions, empty states) *(partial)*

### Phase B — Search quality

1. ~~REQ-SRCH-11 optional re-ranker~~ *(done)*
2. ~~REQ-EMB-07 cache completeness~~ *(done)*
3. ~~Query routing (identifier → keyword)~~ *(done — REQ-SRCH-13)*
4. REQ-SRCH-12 language-aware chunking (Go, TS, Python first) *(partial)*
5. Relevance dashboarding for `semidx bench` in CI/nightly *(todo)*

### Phase C — Hardening

1. Chat/RAG and secondary HTTP entrypoints: always auth + project validation *(partial)*
...
1. ~~REQ-OPS-03 richer metrics~~ *(done)*
2. ~~REQ-SRV-12 global resource caps + enforcement~~ *(done — per-project DB caps remain)*
3. ~~REQ-STOR-06 broader pagination parity~~ *(done)*
4. Large corpus validation and perf budget checks *(todo)*
5. ~~SQLite scale warning~~ *(done)*

### Phase E — Factory / contributor DX

1. REQ-OPS-08 main CI triggers *(todo — `workflow_dispatch` workaround in ci.yml)*
2. ~~REQ-MCP-05 harness gate~~ *(done)*
3. ~~Codex MCP install drift~~ *(done 2026-07-13)*
4. OpenAPI as single contract source *(todo)*
5. Store interface segregation *(todo)*
6. SPA Vitest foundation + E2E Playwright *(partial — Vitest smoke done)*
