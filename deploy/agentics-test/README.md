...
1. **Client wiring gate** — `semidx mcp install --apply` for every supported
   ...
   GitHub Copilot, VS Code, OpenCode, Crush, Pi, Kimi, MiMo) and asserts the written
   config matches that agent's schema; Codex uses TOML `--apply`; cagent is verified
   as print-only; a merge test proves an existing server is preserved and a `.bak`
   is written. See the full list with `semidx mcp install --list`.
2. **Skill install** — `semidx skills install` writes all six bundled skills
   (`semantic-search`, `auto-index`, `workspace-agent`, `semantic-graph`,
   `code-intel`, `impact-before-refactor`).
